...
public class MakeupAdapter extends RecyclerView.Adapter<MakeupAdapter.VH> {

    public interface OnBookClickListener {
        void onBookClick(Makeup item);
    }

    private final List<Makeup> data;
    private final OnBookClickListener listener;
    private final NumberFormat currency = NumberFormat.getNumberInstance(new Locale("en", "PK"));

    public MakeupAdapter(List<Makeup> data, OnBookClickListener listener) {
        this.data = data;
    ...
    public VH onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(parent.getContext())
                .inflate(R.layout.item_makeup, parent, false); // make sure you have this XML
        return new VH(v);
    }

    @Override
    public void onBindViewHolder(@NonNull VH h, int position) {
        Makeup item = data.get(position);
